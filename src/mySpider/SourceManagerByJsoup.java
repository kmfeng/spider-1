...
public class SourceManagerByJsoup {
	...
	/*
	 * 判断 html 页面的类别，根据源码中是否含有 side-anchor 字符串，没有的是普通页面
	 * 1. 普通页面 --> true
	 * 2. 权威医学科普传播网络平台页面 --> false
	 */
...
//		Element nameEle = doc.getElementsByAttributeValue("class", "lemmaTitle").first();
		// 因为 html 的不规范，不一定 lemmaTitle 正好就是 class 的值，但是只要包含 lemmaTitle 就是题目
		Element nameEle = doc.select("[class*=lemmaTitle]").first();
		if (nameEle != null) {
			// 因为包含 lemmaTitle 的String可能不只有题目，所以用空格 spilt 之后取其首字符串
...
//			System.out.println("----------------------------> " + sizeEachList.get(i));
			...
			for (int j = 0; j < sizeEachList.get(i); ++j) {
				...
				System.out.println("getContentDetail +++>>> " + "第 " + i + "组 ----> " + details.get(j + jj).text());
			}
